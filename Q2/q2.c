...
uint64_t x_mul_y_add_a_add_b(uint32_t x, uint32_t y, uint32_t a, uint32_t b)
{
    if ((y | a | b) == 0)
        return 0;
    ...
    return (x_mul_y_add_a_add_b(x, y >> 1, sum >> 1, carry) << 1) | (sum & 1);
}
...
int main(void)
{
    for (int i = 0; i < 100; ++i) {
        ...
        uint64_t xy = x_mul_y_add_a_add_b(x, y, 0, 0);
